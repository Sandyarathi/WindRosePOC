...
void aggData(string fileName, string stationId, int tid, int wr[][NUM_OF_SPEED][NUM_OF_MAX_THREADS]);
	...
	do{
			...
			#pragma omp parallel shared(local_wr)
			{
				...
				for(int i=0; i< vectorOfFilePaths.size(); i++){
					aggData(path + vectorOfFilePaths[i],stationId, tid, local_wr);
					//aggData("../Dataset/07-01_mesonet-20010701_2200.csv", stationId, tid, local_wr);
				}
